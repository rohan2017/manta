...
class Chart(Base):
    error_model='affine_gyro_and_gravity_chart_experiment'
    ...
    def _plus(self,x,d):
        ...
        for slot in self.biases:
            if slot.name.endswith('gyro_bias'):out[_ambient(slot)]=x[_ambient(slot)]+d[_tangent(slot)]
    ...
    def _minus(self,a,b):
        ...
        for slot in self.biases:
            if slot.name.endswith('gyro_bias'):out[_tangent(slot)]=a[_ambient(slot)]-b[_ambient(slot)]
