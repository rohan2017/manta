...
def _port_abi(port, ctx, *, decl: bool = False) -> _PortABI:
    ...
    return for_role(port.role, {
        ...
        Role.STATE: lambda: _PortABI(
            "const State& x" if (port is ctx.x_port and not ctx.held)
            else f"const State& {_ident(port.name)}",
            "x" if (port is ctx.x_port and not ctx.held) else _ident(port.name),
            "x_in" if (port is ctx.x_port and not ctx.held) else f"{_ident(port.name)}_in"),
...
def _manifold_reads(ep, ctx) -> list[tuple[str, str]]:
    ...
    for a in ep.args:
        if isinstance(a, StateRef) \
            ...
            pair = (("x", "x_in") if (port is ctx.x_port and not ctx.held)
...
def emit_module_cpp(module, out_dir, *, class_name: str,
    ...
    C = ["// Generated by manta.codegen. Do not edit by hand.", "",
         f'#include "{base}.hpp"', "", "#include <Eigen/Dense>", "#include <stdexcept>", "",
    ...
    if ctx.held:
        ...
        if ctx.physical_prior:
            init = ["initialize_prior(State{}, prior_P_default());"]
        C += [f"{q}::{q}() {{ {' '.join(init)} }}", ""]
        if ctx.mats:
            ...
            if ctx.physical_prior:
                guards = [f'if ({mf.name}0.cwiseAbs().maxCoeff() != 0.0) '
                          'throw std::invalid_argument("physical prior requires independent nuisance parameters; use restore for checkpoints");'
                          for mf in ctx.mats if mf.name != "P"]
                sets = " ".join(guards + ["initialize_prior(x0, P0);"])
            if ctx.has_clock:
