"""MagneticSource — a part that emits a body-fixed magnetic dipole.

Models the magnetic signature of motors, permanent magnets, or
magnetized structure on a vehicle. A magnetometer on another craft (or
elsewhere on the same craft) reads the dipole field, which swings as the
source craft turns — the moment is body-fixed and rotates with it.
"""
...
class MagneticSource(FieldSource):
    """Emits a magnetic dipole that rides the carrying craft.
