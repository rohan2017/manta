...
void capture_connect_demo_telemetry(const ConnectDemoCraft& craft, double t_sec, ConnectDemoCraftTelemetry& telem);

inline std::string ConnectDemoCraftTelemetry::to_json() const {
    char buf[256];
    std::string s = "{";
    ...
    s += buf;
    s += ",\"leader\":{";
    { char b[64]; std::snprintf(b, sizeof(b), "\"throttle\":%.6f", (double)leader.throttle); s += b; }
    s += "}";
    s += ",\"follower\":{";
    { char b[64]; std::snprintf(b, sizeof(b), "\"throttle\":%.6f", (double)follower.throttle); s += b; }
    s += "}";
    s += "}";
    return s;
}

inline void capture_connect_demo_telemetry(const ConnectDemoCraft& craft, double t_sec, ConnectDemoCraftTelemetry& telem) {
    telem.t_sec = t_sec;
    ...
    telem.leader.throttle = craft.leader().throttle();
    telem.follower.throttle = craft.follower().throttle();
}
