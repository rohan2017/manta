// ex6 — sim + EKF side-by-side, both publishing to Zenoh.
//
...
//   * craft.py     → Ex6Craft       (sim, runs in a Scene)
//   * est_craft.py → Ex6EstCraftT<S> (estimator, wrapped in CraftEKF)
//
...
#include "ex6.hpp"           // Ex6Craft (sim)
#include "ex6_telemetry.hpp"
#include "ex6_est.hpp"       // Ex6EstCraftT<Scalar> (estimator)
...
int main() {
    ...
    Ex6EstCraftT<double>::RigidState x0;
    x0.setZero();
    ...
    auto thrust_sub = session.declare_subscriber(
        zenoh::KeyExpr("manta/ex6/thrust/cmd"),
    ...
    auto state_pub    = session.declare_publisher(zenoh::KeyExpr("manta/ex6/state"));
    auto estimate_pub = session.declare_publisher(zenoh::KeyExpr("manta/ex6/estimate"));

    std::printf("ex6: ready. truth on 'manta/ex6/state', estimate on "
                "'manta/ex6/estimate'.\n");
