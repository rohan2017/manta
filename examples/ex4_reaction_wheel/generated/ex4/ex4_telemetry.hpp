...
void capture_ex4_telemetry(const Ex4Craft& craft, double t_sec, Ex4CraftTelemetry& telem);

inline std::string Ex4CraftTelemetry::to_json() const {
    char buf[256];
    std::string s = "{";
    ...
    s += buf;
    s += ",\"wheel\":{";
    { char b[64]; std::snprintf(b, sizeof(b), "\"angle\":%.6f", (double)wheel.angle); s += b; }
    { char b[64]; std::snprintf(b, sizeof(b), ",\"rate\":%.6f", (double)wheel.rate); s += b; }
    { char b[64]; std::snprintf(b, sizeof(b), ",\"accel\":%.6f", (double)wheel.accel); s += b; }
    s += "}";
    s += "}";
    return s;
}

inline void capture_ex4_telemetry(const Ex4Craft& craft, double t_sec, Ex4CraftTelemetry& telem) {
    telem.t_sec = t_sec;
    ...
    telem.wheel.angle = craft.wheel().angle();
    telem.wheel.rate = craft.wheel().rate();
    telem.wheel.accel = craft.wheel().accel();
}
