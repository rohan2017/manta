...
Pretends to be a robot's sensor stack. Publishes:
  ...
  - manta/ex6/dvl  : 3 floats [vx, vy, vz]              (50 Hz)
...
Usage:
    python examples/ex6_real_data_estimator/fake_sensors.py [--seconds N]
"""
...
def main() -> int:
    ...
    imu_pub = session.declare_publisher("manta/ex6/imu")
    dvl_pub = session.declare_publisher("manta/ex6/dvl")

    rng = random.Random(7)

    print(f"fake_sensors: publishing manta/ex6/imu (250 Hz), "
          f"manta/ex6/dvl (50 Hz). truth vx={args.vx} m/s.")
