...
`manta/ex7/estimate`, and verifies the EKF velocity estimate converged on
the known truth (`vx = 1.5 m/s`, others zero).

Usage:
    Terminal A: ./build/examples/ex7_real_data_estimator
    Terminal B: python examples/ex7_real_data_estimator/fake_sensors.py
    Terminal C: python examples/ex7_real_data_estimator/smoke_test.py
"""
...
def main() -> int:
    ...
    sub = session.declare_subscriber("manta/ex7/estimate", on_estimate)
