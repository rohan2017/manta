...
int main() {
    ...
    Ex5Craft sim_ex5;
    sim_scene.add_craft(sim_ex5, manta::InitialState{});
    ...
    ekf_0.register_field(sim_field_0);
    ...
    auto pub_1 = session.declare_publisher(zenoh::KeyExpr("manta/ex5/state"));
    auto pub_2 = session.declare_publisher(zenoh::KeyExpr("manta/ex5/estimate"));

    std::printf("ex5: ready (sim+EKF). 1 sim craft, 1 est craft, 3 binding(s), 2 measurement sensor(s).\n");
    ...
    while (g_run.load()) {
        { std::lock_guard<std::mutex> lk(bind_0_mtx);
          if (bind_0_payload.size() >= 1) {
              sim_ex5.thrust().set_throttle(bind_0_payload[0]);    // member: set_throttle
        ...
        {
            const double v0 = double(sim_ex5.imu().last_accel().raw()(0));
            const double v1 = double(sim_ex5.imu().last_accel().raw()(1));
            const double v2 = double(sim_ex5.imu().last_accel().raw()(2));
        ...
        {
            const double v0 = double(sim_ex5.imu().last_gyro().raw()(0));
            const double v1 = double(sim_ex5.imu().last_gyro().raw()(1));
            const double v2 = double(sim_ex5.imu().last_gyro().raw()(2));
        ...
        {
            const double v0 = double(sim_ex5.dvl().last_velocity().raw()(0));
            const double v1 = double(sim_ex5.dvl().last_velocity().raw()(1));
            const double v2 = double(sim_ex5.dvl().last_velocity().raw()(2));
        ...
        {
            const double v0 = double(sim_ex5.thrust().throttle());
            ekf_0.craft().thrust().set_throttle(v0);
        }
        ...
        if (++pub_decim >= pub_every) {
            ...
            { std::string _json = "{";
              _json += "\"p\":[";
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", "", double(sim_ex5.scene_to_craft().position().raw()(0))); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().position().raw()(1))); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().position().raw()(2))); _json += _b; }
              _json += "]";
              _json += ",";
              _json += "\"q\":[";
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", "", double(sim_ex5.scene_to_craft().orientation().raw().w())); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().orientation().raw().x())); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().orientation().raw().y())); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().orientation().raw().z())); _json += _b; }
              _json += "]";
              _json += ",";
              _json += "\"v\":[";
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", "", double(sim_ex5.scene_to_craft().vel_linear().raw()(0))); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().vel_linear().raw()(1))); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().vel_linear().raw()(2))); _json += _b; }
              _json += "]";
              _json += ",";
              _json += "\"w\":[";
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", "", double(sim_ex5.scene_to_craft().vel_angular().raw()(0))); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().vel_angular().raw()(1))); _json += _b; }
              { char _b[32]; std::snprintf(_b, sizeof(_b), "%s%g", ",", double(sim_ex5.scene_to_craft().vel_angular().raw()(2))); _json += _b; }
              _json += "]";
              _json += "}";
