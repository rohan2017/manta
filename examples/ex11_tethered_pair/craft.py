"""ex11 — multi-craft tether codegen demo.
...
Regenerate from the repo root:
    ...
    PYTHONPATH=python/manta_codegen/src python -m manta_codegen.cli \
        examples/ex11_tethered_pair/craft.py --workflow binary \
        --out examples/ex11_tethered_pair/generated/ex11
"""
...
def make_drone(name: str, with_thruster: bool) -> Craft:
    ...
    if with_thruster:
        ...
        c.add(thr)
        c.subscribe(thr.set_throttle, "manta/ex11/leader/cmd")
        c.publish({
            "t": c.time,
            "p": c.position, "v": c.vel_linear,
            "throttle": thr.throttle,
        }, f"manta/ex11/{name}/state")
    else:
        c.publish({
            "t": c.time,
            "p": c.position, "v": c.vel_linear,
