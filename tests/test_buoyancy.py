"""FluidField + PointBuoy — Archimedes' principle in manta."""
...
def test_buoy_neutral_when_displaced_weight_equals_craft_weight():
    ...
    np.testing.assert_allclose(cw.state["float"]["position"],
...
def test_buoy_sinks_when_displaced_weight_less_than_craft_weight():
    ...
    assert np.isclose(cw.state["sinker"]["position"][2], 10.0 - expected_drop, atol=1e-4)
