...
class PlanetCoRotatingFluid(Disturbance):
    """Fluid attached to a rotating planet — at rest in the planet's
    frame, moving with `ω × (point − planet.position)` in WorldFrame.

    Spatial extent is selected by a `bound` mode:
      * `"below_sea_level"`  — density present only when the point lies
                               below the planet's sea-level sphere
                               (distance from planet center < radius +
                               sea_level). Used for an ocean.
      * `"above_sea_level"`  — density present only above sea level.
                               Used for an atmosphere.

    The boundary uses a softplus-style smoothing (max(0,x) via
    `0.5(x + sqrt(x²+ε))`) so Jacobians stay regular across the surface;
    in practice this is exact except in a sub-millimeter neighborhood of
    the boundary.

    Args:
        planet         — owning Planet.
        density        — kg/m³ inside the active region.
        planet_radius  — distance from planet.position to the boundary
                         (m). For Earth's ocean / atmosphere split this
                         is just `R_earth + sea_level`.
        bound          — "below_sea_level" or "above_sea_level".
        eps            — smoothing scale for the boundary, m. Default
                         1.0 (sub-meter blend at the boundary; tighten
                         if needed).
    """
    ...
    def __init__(self,
        ...
        if bound not in ("below_sea_level", "above_sea_level"):
            raise ValueError(
                ...
                f"'below_sea_level' or 'above_sea_level', got {bound!r}")
        self.planet = planet
        self.density = float(density)
    ...
    def contribute_at_sym(self, point, t) -> FluidState:
        # Vector from planet center to query point, in WorldFrame.
        r_world  = point._mx - self.planet.position_world_sym()
        # Co-rotation: v = ω × r.
        omega_w  = self.planet.omega_world_sym()
        v_world  = ca.cross(omega_w, r_world)

        # Indicator: smooth 0/1 select on (r - R_planet) sign.
        r_norm   = ca.sqrt(ca.dot(r_world, r_world) + 1e-30)
        signed   = r_norm - self.planet_radius
        # smooth_max(x) = (x + sqrt(x² + ε²)) / 2.
        smooth_max = lambda x: 0.5 * (x + ca.sqrt(x * x + self.eps * self.eps))
        # weight is 1 inside the active region, 0 outside; transitions
        # smoothly over a band of width ~eps around the boundary.
        if self.bound == "below_sea_level":
            # 1 when signed < 0, 0 when signed > 0.
            depth = smooth_max(-signed)
            weight = depth / (depth + self.eps)
        else:
            # 1 when signed > 0, 0 when signed < 0.
            height = smooth_max(signed)
            weight = height / (height + self.eps)

        return FluidState(
            density  = weight * self.density,
            velocity = _VEC3_ANCHOR.from_mx(weight * v_world),
        )

    def __repr__(self) -> str:
        return (f"<PlanetCoRotatingFluid planet={self.planet.name!r} "
                f"density={self.density} R={self.planet_radius} "
