...
class World:
    ...
    def connect(self, source: BoundSignal, sink: BoundSignal) -> "World":
        ...
        same binary. Codegen emits one assignment per tick after w.update()
        — no Zenoh, no JSON, no mutex. Both signals must already be
        attached to a craft (call c.add(...) first) and must agree on
        n_floats."""
        from .signal import Connection
        self.connections.append(Connection(source=source, sink=sink))
        return self
