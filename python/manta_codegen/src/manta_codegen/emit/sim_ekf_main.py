...
# ---------------------------------------------------------------------------
# Composed harness emit (Phase 3 of the harness redesign).
#
# A `[World, EKF/UKF]` Target produces three composed-harness artifacts on
# top of the per-craft files and the two sub-harness pairs:
#
#   <target>.hpp        — namespace manta_gen::<target>: includes
#                         <sim_world>.hpp + <filter_world>.hpp, declares
#                         setup()/tick()/shutdown().
#
#   <target>.cpp        — composed setup() calls each sub-setup; tick()
#                         calls sim::tick(), then runs cross-world connect
#                         steps, then filter::tick(); shutdown() calls
#                         each sub-shutdown in reverse order.
#
#   <target>_main.cpp   — thin pacing loop on top of composed setup/tick.
#
# Sub-harness ticks each handle their own intra-world connects + bindings.
# The composed tick only handles connections that span the two worlds.
...
def _emit_composed_connect_step(
    ...
    def _accessor(bs, scope_namespace: str, in_filter_world: bool) -> tuple[str, bool]:
        ...
        if in_filter_world:
            base = f"{filter_ns}::{filter_obj.cpp_var_name()}.craft()"
