...
Exercises emit_main_cpp on a synthetic craft. Verifies each binding
produces a publisher/subscriber + main-loop apply/publish snippet.
...
def _wrap(c: Craft) -> World:
    """Wrap a Craft in a minimal World so emit_main_cpp can read the per-craft
    initial state. Tests don't care about pose so we leave the entry at default."""
...
def test_emit_main_no_legacy_state_pub() -> None:
    """The legacy auto-bundled `state_pub` topic is gone. A craft with no
    bindings emits no publishers at all (the user is expected to declare
    explicit bindings)."""
    c = _build_craft_with_bindings()
    src = emit_main_cpp(c, _wrap(c))
    _check("state_pub" not in src,
           "no auto-bundled state_pub should ever be emitted")
    _check("ex_bindTelemetry telem" not in src and "capture_ex_bind_telemetry" not in src,
           "no auto-telemetry capture should be emitted")
