...
public:
    ...
    KinematicLink<From, C, Scalar> operator*(const KinematicLink<To, C, Scalar>& bc) const noexcept {
        ...
        auto p_AC = orientation_.raw() * bc.position().raw() + position_.raw();

        // Linear velocity in A: v_AB + omega_AB x (R_AB * p_BC) + R_AB * v_BC
        auto p_BC_in_A   = orientation_.raw() * bc.position().raw();
        auto omega_AB_A  = orientation_.raw() * vel_angular_.raw();
        auto v_BC_in_A   = orientation_.raw() * bc.vel_linear().raw();
        auto v_AC_A      = vel_linear_.raw() + omega_AB_A.cross(p_BC_in_A) + v_BC_in_A;
        ...
        auto omega_AB_C  = bc.orientation().raw().conjugate() * vel_angular_.raw();
        auto omega_AC_C  = omega_AB_C + bc.vel_angular().raw();
        ...
        auto alpha_AB_A  = orientation_.raw() * acc_angular_.raw();
        auto a_BC_in_A   = orientation_.raw() * bc.acc_linear().raw();
        auto a_AC_A      = acc_linear_.raw()
        ...
        auto alpha_AB_C  = bc.orientation().raw().conjugate() * acc_angular_.raw();
        auto omega_BC_C  = bc.vel_angular().raw();
        auto alpha_AC_C  = alpha_AB_C + bc.acc_angular().raw()
    ...
    KinematicLink<To, From, Scalar> inverse() const noexcept {
        ...
        auto omega_AB_A   = R_AB * vel_angular_.raw();
        auto v_BA         = -(R_BA * (vel_linear_.raw() - omega_AB_A.cross(position_.raw())));
...
// RK4 update implementation. Defined out-of-line for readability.
template <typename From, typename To, typename Scalar>
inline void KinematicLink<From, To, Scalar>::update(Scalar dt) noexcept {
    // Treat velocities as having constant acceleration over the step.
    auto v0 = vel_linear_.raw();
    auto a0 = acc_linear_.raw();
    auto w0 = vel_angular_.raw();
    auto al = acc_angular_.raw();

    // RK4 on position with constant linear acceleration is exact:
    //   p(t+dt) = p + v*dt + 0.5*a*dt^2
    //   v(t+dt) = v + a*dt
    auto p_new = position_.raw() + v0 * dt + Scalar(0.5) * a0 * dt * dt;
    auto v_new = v0 + a0 * dt;

    // Orientation: integrate via small-angle quaternion update using midpoint
    // angular velocity. For real precision we'd want a proper RK4 on the
    // quaternion ODE; this midpoint method is accurate to O(dt^2).
