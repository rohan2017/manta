...
//
// Fields store Real-only disturbance lambdas (lifetime, in_influence,
...
//
// The default path here is finite-difference around the Real value:
//   * Real-Scalar: cast pos→Real, query f.state_at(pos), cast result→Scalar.
//   * Jet-Scalar:  evaluate at the Real value plus 6 axis-aligned ±eps
...
// Default finite-diff step. Tuned conservatively — `Real = float` has
...
inline constexpr Real kFiniteDiffEps = Real(1e-3);
...
//
// `Scalar` may be Real, double, or `ceres::Jet<double, N>`. For floating-point
...
state_at_templated(const FieldT& f,
    ...
    if constexpr (std::is_floating_point_v<Scalar>) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            for (int i = 0; i < 3; ++i) {
                J(i, j) = (vp.raw()(i) - vm.raw()(i)) / (Real(2) * eps);
            }
        }
